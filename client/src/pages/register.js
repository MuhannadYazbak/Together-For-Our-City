...
const Register = ({selectedUser, setSelectedUser}) => {
    ...
    const [personalDetails, setPersonalDetails] = useState(selectedUser ? 
        {
    ...
    const [addressDetails, setAddressDetails] = useState(selectedUser && selectedUser.address ?
        {
          city: selectedUser.address.city,
          neighborhood: selectedUser.address.neighborhood,
          postalCode: selectedUser.address.postalCode,
        } : null);
    const [passwordDetails, setPasswordDetails] = useState(selectedUser ?
        {
    ...
    function disabledStep(stepNo){
        ...
        else return personalDetails === null || addressDetails === null || passwordDetails === null;
    };
            ...
            <Form className="formStyle" onFinish={onFinish} wrapperCol={{offset: 0}}>
                <Form.Item>
                    <Space className="messageBoxStyle" direction="vertical">
                        ...
                        <Button className="buttonStyle" type='default' onClick={()=> navigate('/addAssociation')}>{t('Buttons.addAssoc')}</Button>
                        </Space>
                    </Form.Item>
                    <Form.Item>
    ...
    useEffect(()=>{
        console.log("Register UseEffect : SelectedUser: ", selectedUser, " personal Details: ", personalDetails, " address Details: ", addressDetails, " password Details: ", passwordDetails);
    },[])
    ...
    function PersonalDetailsForm  ({onFinish, initialValues}) {
        return (
            <Form className="formStyle" onFinish={onFinish} initialValues={initialValues} labelCol={{span: 10}} wrapperCol={{span: 10}}>
                <Form.Item label={t('RegForm.firstName')} name='firstName' rules={[{ required: true, message: t('Messages.firstName')}]}>
                    <Input name='firstName' type='text' />
                </Form.Item>
                <Form.Item label={t('RegForm.lastName')} name='lastName' rules={[{ required: true, message: t('Messages.lastName')}]}>
                    <Input name='lastName' type='text' />
                </Form.Item>
                <Form.Item label={t('RegForm.email')} name='email' rules={[{ required: true, message: t('Messages.email')}]}>
                    <Input name='email' type='email' />
                </Form.Item>
                <Form.Item label={t('RegForm.phone')} name='phone' rules={[{ required: true, message: t('Messages.phone')}]}>
    ...
    function AddressDetailsForm  ({onFinish, initialValues}) {
        return (
            <Form className="formStyle" onFinish={onFinish} initialValues={initialValues} labelCol={{span: 10}} wrapperCol={{span: 10}}>
                <Form.Item label={t('RegForm.Address.city')} name='city'  rules={[{ required: true, message: t('Messages.city')}]}>
                    <Input type='text' name='city' />
                </Form.Item>
                <Form.Item label={t('RegForm.Address.neighborhood')} name='neighborhood' rules={[{ required: true, message: t('Messages.neighborhood')}]}>
                    <Input type='text' name='neighborhood' />
                </Form.Item>
                <Form.Item label={t('RegForm.Address.postalCode')} name='postalCode' rules={[{ required: false, message: t('Messages.postalCode')}]}>
    ...
    function PasswordDetailsForm  ({onFinish, initialValues}) {
        return (
            <Form className="formStyle" onFinish={onFinish} initialValues={initialValues} labelCol={{span: 10}} wrapperCol={{span: 10}}>
                <Form.Item label={t('RegForm.password')} name='password'  rules={[{ required: true, message: t('Messages.password')}]}>
                    <Input type='password' name='password' />
                </Form.Item>
                <Form.Item label={t('RegForm.repassword')} name='repassword'  rules={[{ required: true, message: t('Messages.repassword')}]}>
    ...
    const onFinishLastStep = async () => {
        ...
        if (localStorage.getItem('token')){
           ...
           await axios.put(`http://localhost:3001/users/${selectedUser._id}`, updatedUser).then((response)=>{
        ...
        await axios.post("http://localhost:3001/register", updatedUser).then((response)=>{
                if (response.status == 200){
    ...
    function LastForm ({onFinish}){
        return (
            <Form className="formStyle" onFinish={onFinish}>
                ...
                <Form.Item wrapperCol={{offset: 0}}>
                    <Button className="buttonStyle" type='primary' htmlType="submit">{t('Buttons.addUser')}</Button>
                </Form.Item>
            </Form>
        )
    }
    ...
    const forms = [
        ...
        <PersonalDetailsForm onFinish={onFinishStepOne} initialValues={personalDetails} />,
        <AddressDetailsForm onFinish={onFinishStepTwo} initialValues={addressDetails} />,
        <PasswordDetailsForm onFinish={onFinishStepThree} initialValues={passwordDetails} />,
    ...
    <Space direction="vertical">
        ...
        <Steps className='stepsStyle' onChange={setCurrent} current={current} items={[
            ...
            { title: t('Steps.userPersonal'), icon: <InfoCircleOutlined />, disabled: disabledStep(1) },
            { title: t('Steps.userAddress'), icon: <HeatMapOutlined />, disabled: disabledStep(2) },
            { title: t('Steps.userPassword'), icon: <ContactsOutlined />, disabled: disabledStep(3) },
